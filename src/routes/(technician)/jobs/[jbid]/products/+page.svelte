  ...
  async function fetchInstallations() {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/installations/?page=1&limit=100&job_id=${JOB_ID}`,
...
<div class="space-y-6">
    ...
    {#if selectedProducts.length}
      <div class="mt-8">
        ...
        <form method="POST" action="?/saveProducts" use:enhance={() => {
          ...
          <input type="hidden" name="projectId" value={JOB_ID} />
          <input
            type="hidden"
            name="products"
                ...
                {#each selectedProducts as product}
                  <tr class="border-b last:border-0 hover:bg-gray-50 transition-colors">
                    ...
                    <td class="p-3 text-right">
                      <button type="button" class="text-red-500 hover:text-red-700 font-medium" onclick={() => removeProduct(product.id)}>
                        Remove
                      </button>
                    </td>
                  </tr>
          ...
          <div class="flex items-center justify-between mt-4">
              ...
              {/if}
              <button type="submit" class="ml-auto bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 font-semibold shadow-md transition-all active:scale-95">
  ...
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
    <h2 class="text-lg font-semibold text-gray-800 mb-4">Current Installations (Job: {JOB_ID})</h2>

    {#if loading}
      <div class="flex items-center gap-2 text-gray-500 italic"><span class="animate-pulse">Loading...</span></div>
    {:else if error}
      <div class="p-3 bg-red-50 text-red-700 rounded-lg border border-red-100">{error}</div>
    {:else if installations.length === 0}
      <p class="text-gray-500 py-4">No installations found for this job.</p>
    {:else}
      <div class="border border-gray-200 rounded-lg overflow-hidden">
            ...
            {#each installations as item (item.id)}
              <tr class="border-b last:border-0 hover:bg-gray-50 transition-colors">
                <td class="p-3">
                  ...
                  <div class="text-[10px] text-gray-400 uppercase tracking-tighter">ID: {item.prd_id}</div>
                </td>
