<script lang="ts">
  ...
  import { ArrowLeft, MapPin, Calendar, User, Briefcase, ChevronDown } from "@lucide/svelte";
...
<div class="mb-6 flex items-center gap-4">
  <button class="btn btn-circle btn-ghost bg-base-200" on:click={() => goto(`/dashboard/projects/${projectID}`)}>
    <ArrowLeft class="w-5 h-5" />
  </button>
  <div>
    <h1 class="text-3xl font-bold text-base-content">Create Team Lead Job</h1>
    <p class="text-sm text-base-content/60 mt-1">Assign a new job and select a team lead</p>
  </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
  <div class="card bg-base-100 shadow-xl border border-base-200 lg:col-span-2">
    <div class="card-body">
      <h2 class="card-title text-xl font-semibold border-b border-base-200 pb-2 mb-4">Job Details</h2>
      ...
      >
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="form-control w-full">
            <label class="label" for="start_date">
              ...
              <span class="label-text-alt text-base-content/60">America/New_York</span>
            </label>
            <div class="relative">
              <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-base-content/50">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="form-control w-full relative">
            ...
            {#if jobtype.length}
              <div class="absolute top-20 z-10 bg-base-100 shadow-2xl rounded-xl w-full border border-base-300 max-h-60 overflow-auto">
          ...
          <div class="form-control w-full relative">
            <label class="label" for="employee_search">
              <span class="label-text font-medium text-base-content/80">Team Lead (Employee)</span>
            </label>
            <div class="relative">
              <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-base-content/50">
                <User class="h-5 w-5" />
        ...
        <div class="flex justify-end gap-3 mt-8">
          <button type="button" class="btn btn-ghost" on:click={() => goto(`/dashboard/projects/${projectID}`)}>
            Cancel
          </button>
  ...
  <div class="card bg-base-100 shadow-xl border border-base-200 h-fit">
    <div class="card-body">
      ...
      <ul class="list-disc list-inside text-sm text-base-content/80 space-y-2">
        ...
        <li>Ensure the correct Job Type is selected from the dropdown so standard pay rates can apply.</li>
        <li>Start dates indicate when the technician should be on site. All times are America/New_York.</li>
      </ul>
        ...
        <div class="bg-primary/10 rounded-xl p-4 border border-primary/20">
          <div class="flex items-center gap-3">
            ...
            <div class="flex flex-col">
              <span class="font-bold text-base-content text-sm">Selected Lead</span>
      ...
      {:else}
        <div class="alert mt-2 py-3 px-3 rounded-lg text-sm bg-base-200 border-none flex flex-row items-center gap-3">
          ...
          <span>No employee selected yet. Ensure you assign a Team Lead before submitting.</span>
        </div>
      {/if}
    </div>
  </div>
