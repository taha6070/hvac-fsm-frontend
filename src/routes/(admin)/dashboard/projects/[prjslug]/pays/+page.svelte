<script lang="ts">
    ...
    async function TechnicianPayUpdate(id: Number) {
        try {
            const response = await fetch(`${PUBLIC_API_URL}/pays/${id}`, {
                method: "PATCH",
                headers: {
                    Accept: "application/json",
                    "Content-Type": "application/json",
    ...
    async function ContractorPayUpdate(id: Number) {
        try {
            const response = await fetch(
                `${PUBLIC_API_URL}/contractor/pays/${id}`,
                {
                    method: "PATCH",
                    headers: {
                        Accept: "application/json",
...
<div class="p-6 max-w-7xl mx-auto space-y-8">
    ...
    >
        <div
            class="bg-primary/10 px-6 py-4 flex justify-between items-center border-b border-base-300"
        >
            <h2 class="text-lg font-bold flex items-center gap-2 text-primary">
                <Wallet size={18} /> Contractor Payroll
            </h2>
            <span class="badge badge-primary"
                >{contractorPays.length} Records</span
            >
        </div>
            ...
            <table class="table table-zebra w-full">
                ...
                <tbody>
                    {#each contractorPays as pay}
                        <tr>
                            <td class="font-bold"
                                >{pay.employee || pay.employee_name}</td
                            >
                            <td class="text-info"
                                >{formatCurrency(pay.calculated_amount)}</td
                            >
                            <td class="font-semibold"
                                >{formatCurrency(pay.total_amount)}</td
                            >
                            <td>
                                {#if pay.status}
                                    ...
                                    >
                                {:else}
                                    <span class="text-error italic text-xs"
                                        >Pending</span
                                    >
                                {/if}
                            </td>
                            ...
                            <td class="text-right">
                                <button
                                    ...
                                    on:click={() =>
                                        ContractorPayUpdate(pay.pay_id)}
                                    >Paid</button
                                >
                            </td>
                        </tr>
    ...
    >
        ...
        >
            ...
            <span class="badge badge-primary"
                >{standardPays.length} Records</span
            >
        </div>
        <div class="overflow-x-auto">
            <table class="table table-zebra w-full">
                        ...
                        <tr>
                            ...
                            <td class="text-right">
                                <button
                                    class="btn btn-ghost btn-xs text-secondary"
                                    on:click={() =>
                                        TechnicianPayUpdate(pay.pay_id)}
                                    >Paid</button
                                >
                                {pay.pay_id}
                            </td>
    ...
    >
        <div
            class="bg-secondary/10 px-6 py-4 flex justify-between items-center border-b border-base-300"
        >
            <h2
                class="text-lg font-bold flex items-center gap-2 text-secondary"
            >
                <Users size={18} /> Technician/Helper Pays
            </h2>
            <span class="badge badge-secondary"
                >{helperPays.length} Records</span
            >
        </div>
        <div class="overflow-x-auto">
            <table class="table table-zebra w-full">
                ...
                <tbody>
                    {#each helperPays as pay}
                        <tr>
                            <td class="font-bold"
                                >{pay.employee_name || pay.emp_name}</td
                            >
                            ...
                            <td class="text-right">
                                <button
                                    class="btn btn-ghost btn-xs text-secondary"
                                    on:click={() => HelperPayUpdate(pay.pay_id)}
                                    >Paid</button
                                >
                            </td>
                        </tr>
                    {:else}
                        <tr
                            ><td colspan="5" class="text-center py-4 opacity-50"
                                >No technician payroll found.</td
                            ></tr
                        >
                    {/each}
                </tbody>
