<script lang="ts">
    ...
    searchTimeout = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `${PUBLIC_API_URL}/contacts?page=1&limit=10&name=${encodeURIComponent(searchQuery)}`,
        );
        if (res.ok) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      errorMessage =
        err.message || "An error occurred while saving the project.";
    } finally {
      isSaving = false;
    }
  };
...
<div class="mb-6 flex items-center gap-4">
  <button
    ...
    on:click={() => goto(`/dashboard/projects/${page.params.prjslug}`)}
  >
    ...
    <div class="card-body">
      ...
      <div class="form-control w-full">
        {#if selectedCustomer}
          ...
          <div
            class="bg-base-200/50 rounded-xl p-4 border border-base-300 relative group transition-all hover:bg-base-200"
          >
            <button
              class="absolute top-2 right-2 btn btn-xs btn-circle btn-ghost text-base-content/50 hover:text-error opacity-0 group-hover:opacity-100 transition-opacity"
            ...
            <div class="flex items-center gap-3">
              <div class="avatar placeholder">
                <div class="bg-primary text-primary-content rounded-full w-10">
                  <span class="text-sm font-bold"
                    >{selectedCustomer.name.charAt(0).toUpperCase()}</span
                  >
                </div>
              </div>
              <div class="flex flex-col">
                ...
                <span class="text-xs text-base-content/60"
                  >{selectedCustomer.phone_no || "No phone"}</span
                >
                {#if selectedCustomer.email}
                  <span class="text-xs text-base-content/60 truncate w-40"
            ...
            <div
              class="mt-3 pt-3 border-t border-base-300/50 text-xs text-center text-success font-medium"
            >
              ✓ Attached to project
            </div>
          </div>
        {:else}
          ...
          <div class="relative">
            ...
            {#if showDropdown && searchResults.length > 0}
              <ul
                class="absolute z-10 mt-1 w-full bg-base-100 border border-base-300 shadow-2xl max-h-60 rounded-box overflow-auto"
              >
                {#each searchResults as contact}
                  <li>
                    <button
                      class="w-full text-left px-4 py-3 hover:bg-base-200 transition-colors border-b border-base-200 last:border-0 flex items-center gap-3"
                      type="button"
                      on:click={() => selectCustomer(contact)}
                    >
                      <div
                        class="bg-base-300 rounded-full p-2 text-base-content/70"
                      >
                        <User class="w-4 h-4" />
                      </div>
                      <div class="flex flex-col">
            ...
            {:else if showDropdown && searchResults.length === 0 && searchQuery.length > 0 && !isSearching}
              <div
                class="absolute z-10 mt-1 w-full bg-base-100 border border-base-300 shadow-lg p-4 rounded-box text-center text-sm text-base-content/60"
              >
          ...
          <div
            class="alert mt-4 py-2 px-3 rounded-lg text-sm bg-info/10 text-info border-none"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
