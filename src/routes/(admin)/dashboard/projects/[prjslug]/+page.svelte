...
<div class="mb-6 flex justify-between items-center">
  <div class="flex items-center gap-4">
    <button class="btn btn-circle btn-ghost bg-base-200" on:click={() => goto('/dashboard/projects')}>
      <ArrowLeft />
    </button>
    <h1 class="text-3xl font-bold text-base-content">Project Details</h1>
  </div>
  <div class="flex gap-2">
    <button class="btn btn-outline" on:click={() => goto(`/dashboard/projects/${page.params.prjslug}/edit`)}>
  ...
  <div class="card bg-base-100 shadow-xl border border-base-200 lg:col-span-2">
    <div class="card-body">
      <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
        ...
        <div class="badge badge-lg" class:badge-success={project.status === 'done'} class:badge-warning={project.status === 'inprogress'} class:badge-info={project.status === 'notstart'} class:badge-ghost={project.status !== 'done' && project.status !== 'inprogress' && project.status !== 'notstart'}>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div class="flex flex-col">
          <span class="text-xs font-semibold uppercase text-base-content/50 mb-1">Start Date</span>
        ...
        <div class="flex flex-col">
          <span class="text-xs font-semibold uppercase text-base-content/50 mb-1">End Date</span>
          <span class="font-medium flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-error" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
  ...
  <div class="card bg-base-100 shadow-xl border border-base-200">
    <div class="card-body">
      ...
      <div class="flex flex-col gap-2">
        <button class="btn btn-primary btn-sm justify-start" on:click={() => goto(`/dashboard/projects/${page.params.prjslug}/jobs/create`)}>
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
          Add Team Lead Job
        </button>
        <button class="btn btn-secondary btn-sm justify-start" on:click={() => goto(`/dashboard/projects/${page.params.prjslug}/helperjobs/create`)}>
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
          Add Tech/Helper Job
        </button>
        <div class="divider my-0"></div>
        <button class="btn btn-accent btn-sm justify-start" on:click={() => generatePays(projectId)}>
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          Generate Pay
        </button>
        <button class="btn btn-outline btn-sm justify-start" on:click={() => goto(`/dashboard/projects/${page.params.prjslug}/pays`)}>
